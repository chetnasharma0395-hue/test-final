export const jimCorbettFAQs = [
  {
    question: 'What is the taxi fare from Dehradun to Jim Corbett National Park in 2026?',
    answer:
      'The fixed one-way taxi fare from Dehradun to Jim Corbett National Park (Ramnagar) is ₹4,000 for a Sedan and ₹5,000 for an SUV with Uttarakhand Cab 24/7. The 200 km journey via NH74 takes approximately 5–6 hours. Pickup available from Dehradun city, Railway Station, ISBT, or Jolly Grant Airport.',
  },
  {
    question: 'What is the taxi fare from Delhi to Jim Corbett National Park?',
    answer:
      'The fixed one-way taxi fare from Delhi to Jim Corbett National Park (Ramnagar) is ₹5,000 for a Sedan and ₹6,000 for an SUV with Uttarakhand Cab 24/7. The 260 km journey via NH9 takes approximately 6–7 hours depending on Delhi traffic. Pickup from any Delhi address, IGI Airport, or NDLS Railway Station.',
  },
  {
    question: 'What is the taxi fare from Rishikesh to Jim Corbett?',
    answer:
      'The fixed one-way taxi fare from Rishikesh to Jim Corbett National Park is ₹4,000 for a Sedan and ₹4,500 for an SUV. The 180 km route via Kotdwar takes approximately 4.5–5 hours. This is a popular route for travellers combining a Rishikesh yoga retreat with a Jim Corbett wildlife safari.',
  },
  {
    question: 'What is the taxi fare from Mussoorie to Jim Corbett?',
    answer:
      'The fixed taxi fare from Mussoorie to Jim Corbett National Park is ₹4,500 for a Sedan and ₹5,500 for an SUV. The 230 km journey via Dehradun and NH74 takes approximately 5.5–6.5 hours. A great option for travellers combining a Mussoorie hill holiday with a Corbett jungle safari.',
  },
  {
    question: 'Can I book a round-trip taxi to Jim Corbett from Dehradun?',
    answer:
      'Yes. Uttarakhand Cab 24/7 offers round-trip taxi packages to Jim Corbett from Dehradun, Delhi, Rishikesh, and Mussoorie. Round-trip fares are calculated based on total distance and days. For a 2-day Corbett trip from Dehradun, a round-trip Sedan package starts at ₹7,000 including driver allowance. WhatsApp +91 92589 12169 for a custom quote.',
  },
  {
    question: 'What is the taxi fare from Delhi to Jim Corbett National Park?',
    answer:
      'The fixed one-way taxi fare from Delhi to Jim Corbett National Park (Ramnagar) is ₹4,500 for a Sedan and ₹5,500 for an SUV with Uttarakhand Cab 24/7. The 260 km journey via NH9 takes approximately 6–7 hours depending on Delhi traffic. Pickup from any Delhi address, IGI Airport, or NDLS Railway Station.',
  },
  {
    question: 'What is the taxi fare from Rishikesh to Jim Corbett?',
    answer:
      'The fixed one-way taxi fare from Rishikesh to Jim Corbett National Park is ₹3,000 for a Sedan and ₹4,000 for an SUV. The 180 km route via Kotdwar takes approximately 4.5–5 hours. This is a popular route for travellers combining a Rishikesh yoga retreat with a Jim Corbett wildlife safari.',
  },
  {
    question: 'What is the taxi fare from Mussoorie to Jim Corbett?',
    answer:
      'The fixed taxi fare from Mussoorie to Jim Corbett National Park is ₹4,000 for a Sedan and ₹5,000 for an SUV. The 230 km journey via Dehradun and NH74 takes approximately 5.5–6.5 hours. A great option for travellers combining a Mussoorie hill holiday with a Corbett jungle safari.',
  },
  {
    question: 'Which gate of Jim Corbett National Park should I enter from?',
    answer:
      'Jim Corbett has 5 zones — Dhikala, Bijrani, Jhirna, Dhela, and Durga Devi. Dhikala is the most popular and offers the best chance of spotting tigers but requires advance permit booking. Jhirna zone is open year-round including monsoon. Bijrani is ideal for day safaris. Our drivers know all entry gates and will drop you at your specific zone gate.',
  },
  {
    question: 'What is the best time to visit Jim Corbett National Park?',
    answer:
      'The best time to visit Jim Corbett is November to June. The park is most accessible from November to March when wildlife sightings are highest due to dry grasslands and animals congregating near water sources. April to June is hot but excellent for tiger sightings. Most zones close from July to October during monsoon, except Jhirna which stays open year-round.',
  },
  {
    question: 'Can I book a round-trip taxi to Jim Corbett from Dehradun?',
    answer:
      'Yes. Uttarakhand Cab 24/7 offers round-trip taxi packages to Jim Corbett from Dehradun, Delhi, Rishikesh, and Mussoorie. Round-trip fares are calculated based on total distance and days. For a 2-day Corbett trip from Dehradun, a round-trip Sedan package starts at ₹7,000 including driver allowance. WhatsApp +91 92589 12169 for a custom quote.',
  },
  {
    question: 'Is Uttarakhand Cab 24/7 reliable for Jim Corbett safari trips?',
    answer:
      'Yes. Uttarakhand Cab 24/7 is rated 4.9 stars on Google with 161 verified reviews. Our drivers are experienced on the Dehradun–Corbett route and familiar with all park entry gates and resort locations inside the Corbett buffer zone. We also assist with early morning safari pickups — most safaris start at 6 AM and require on-time arrival at the zone gate.',
  },
];
